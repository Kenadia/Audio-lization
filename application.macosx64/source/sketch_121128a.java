...
public class sketch_121128a extends PApplet {
...
public int [] crop(int [] pix, int image_w, int x, int y, int w, int h) {
  int [] cropped = new int[w * h];
  for (int i = 0; i < w; i++) {
    for (int j = 0; j < h; j++) {
...
public void draw() {
  if (cam.available() == true) {
    ...
    //
    int [] pix = cam.pixels;
    int [] cropped = crop(pix, cam.width, 160, 120, 80, 60);
...
public void audioStreamWrite(AudioStream stream) {
  synth.writeStream(stream);
}

class Synth {
  final float [] hue_set = {0.0f, 0.097f, 0.164f, 0.309f, 0.490f, 0.652f, 0.730f, 0.838f};
//  final float [] fundamentals = {58.2705, 65.4064, 69.2957, 77.7817, 82.4069, 43.6535, 48.9994, 51.9131};
  final float [] fundamentals = {65.4064f, 48.9994f, 73.4162f, 58.2705f, 38.8909f, 51.9131f, 77.7817f, 87.3071f};
  ...
  Synth(AudioStream [] output_streams) {
    ...
    for (int i = 0; i < voice_count; i++) {
      float pan = voice_count == 1? 0.0f : i * 2.f / (voice_count - 1) - 1;
  ...
  public void playImage(int [] pix, int image_w) {
  ...
  public void writeStream(AudioStream stream) {
    int stream_id = -1;
    for (int i = 0; i < voice_count; i++) {
...
class Voice {
  ...
  Voice(AudioStream stream, float [] hue_set, float [] fundamentals) {
    ...
    this.noise_level = 0.0f;
    this.last_noise_level = 0.0f;
    this.silence = new Silence();
    this.pink_noise = new PinkNoise(0.0f);
    this.pixel_count = 0;
  }
  public void startFrame(int pixel_count) {
  ...
  public void addPoint(int y, int c) {
    float hu = hue(c) / 255.f;
    float sa = saturation(c) / 255.f;
    float br = brightness(c) / 255.f;
    float shade = br - sa;
    int overtone;
    if (shade < -0.85f) {
      overtone = -1;
    ...
    if (overtone >= 0) {
      ...
      float left_hue_amt = 0.0f, right_hue_amt = 0.0f;
      for (int i = hue_count - 1; i >= 0; i--) {
        if (hu >= hue_set[i]) {
          left_hue_i = i;
          if (i == hue_count - 1) {
            right_hue_i = 0;
            right_hue_amt = (hu - hue_set[i]) / (1.0f - hue_set[i]);
          } else {
            right_hue_i = i + 1;
            right_hue_amt = (hu - hue_set[i]) / (hue_set[i + 1] - hue_set[i]);
          }
          left_hue_amt = 1.0f - right_hue_amt;
          break;
        }
      }
...
class SynthUnit {
  ...
  SynthUnit(float pitch) {
    this.sine = new SineWave(pitch, 0.0f);
  ...
  public void play(int r, int g, int b) {
    int c = color(r, g, b);
    float hu = hue(c) / 255.f;
    float sa = saturation(c) / 255.f;
    float br = brightness(c) / 255.f;
    colorMode(HSB, 1.0f);
    int hue_color = color(hu, 1.0f, 1.0f);
    colorMode(RGB, 255);
    float hue_r = red(hue_color) / 255.f;
    float hue_g = green(hue_color) / 255.f;
    float hue_b = blue(hue_color) / 255.f;
    float tone_vol;
    if (br >= 0.5f) {
      tone_vol = sa;
    } else {
  ...
  public void writeToStream(AudioStream stream) {
...
class VoiceOne {
  ...
  public void play(int region_y, int r, int g, int b) {
...
class SynthUnitOne {
  ...
  SynthUnitOne(float pitch) {
    this.sine = new SineWave(pitch, 0.0f);
    this.tri = new TriangleWave(pitch, 0.0f);
    this.square = new SquareWave(pitch, 0.0f);
    this.saw = new SawtoothWave(pitch, 0.0f);
    this.sine = new SineWave(pitch, 0.0f);
    this.pink_noise = new PinkNoise(0.0f);
  ...
  public void play(int r, int g, int b) {
    ...
    float tone_vol;
    if (br >= 0.5f) {
      pink_noise.volume = (br - 0.5f) * (1.0f - sa) * 2;
      tone_vol = sa;
    } else {
      pink_noise.volume = 0.0f;
      tone_vol = sa * br * 2;
    }
    tone_vol *= 0.1f;
    saw.volume = hue_r * tone_vol;
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "sketch_121128a" };
    if (passedArgs != null) {
